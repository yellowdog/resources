...
1. The current version of the YellowDog Agent installer can be downloaded from YellowDog's Nexus software repository at: https://nexus.yellowdog.tech/repository/raw-public/agent/msi/yd-agent-17.1.1.msi.
...
Invoke-WebRequest -Uri 'https://nexus.yellowdog.tech/repository/raw-public/agent/msi/yd-agent-17.1.1.msi' -OutFile yd-agent-17.1.1.msi
```

2. In the directory to which the file has been downloaded, run the installer from the command line as Administrator:

```shell
msiexec /i yd-agent-17.1.1.msi /passive /log yd-agent-install.log
```
...
```shell
msiexec /i yd-agent-17.1.1.msi /passive /log yd-agent-install.log YD_AGENT_METADATA_PROVIDERS=AWS
```
